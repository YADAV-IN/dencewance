import { Client, Databases, ID } from 'node-appwrite';

const client = new Client()
  .setEndpoint('https://nyc.cloud.appwrite.io/v1')
  .setProject('69d60fbe002bae1e32d5')
  .setKey('standard_');

const databases = new Databases(client);

async function run() {
  try {
    await databases.createBooleanAttribute('69d60fe8000c9bd92750', 'news', 'is_active', false, true);
    console.log('is_active created on news');
  } catch(e) { if(e.code !== 409) console.log(e.message); }
  
  try {
    await databases.createBooleanAttribute('69d60fe8000c9bd92750', 'admins', 'is_active', false, true);
  } catch(e) { if(e.code !== 409) console.log(e.message); }
}
run();
