...
async function run() {
  try {
    await databases.createBooleanAttribute('69d60fe8000c9bd92750', 'reels', 'is_active', false, true);
    console.log('is_active boolean attribute created on reels collection.');
  } catch (err) {
    if (err.code !== 409) console.error(err);
    else console.log('is_active already exists on reels.');
  }
}
run();
